...
O terceiro é o \texttt{background}, que guarda o fundo da ficha: a folha sobre a qual todos os Elementos são desenhados. É coluna do \textit{layout}, e não chave no \texttt{style} de um Elemento, justamente por não pertencer a Elemento nenhum; e é uma coluna só porque a cópia individual de item é uma linha desta mesma tabela, de modo que o \textit{template} reutilizável e a ficha de cada item ficam cobertos de uma vez. Guarda uma cor em \texttt{\#rrggbb} ou, quando o fundo é um degradê, duas cores com um ângulo e um ponto de início, na forma \texttt{\#rrggbb..\#rrggbb@graus,inicio}. Não se grava ali a regra de estilo pronta: uma declaração de degradê aceita como texto livre seria injeção de código na página de quem visita a ficha, e impediria o cálculo descrito adiante. Cabe ao \textit{front-end} compor a regra a partir dos valores validados.
...
Cada paleta pertence a um usuário e tem nome único dentro de sua conta. O campo \texttt{subcolors} emprega o tipo JSONB e guarda, em lista, as doze subcores da paleta (as cores de cada superfície e o texto de cada uma, conforme o RF-18), sem exigir tabela própria. A tipografia do RF-19 fica na própria paleta: além da família, dos tamanhos e dos espaçamentos, o \texttt{font\_weight} guarda o peso do texto comum, de 300 a 800, do qual os títulos derivam sempre dois degraus acima, e o \texttt{font\_italic}, a inclinação. Os dois indicadores booleanos têm papéis distintos: \texttt{is\_active} aponta a paleta em uso, e \texttt{is\_default} assinala as seis paletas predefinidas que o sistema cria no cadastro (RF-01). Esse indicador marca apenas a origem da paleta, e não a torna imutável: as predefinidas podem ser editadas e excluídas como as demais.
\FloatBarrier
