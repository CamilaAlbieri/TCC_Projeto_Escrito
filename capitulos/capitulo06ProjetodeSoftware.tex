...
Cada paleta pertence a um usuário, e seu nome não se repete entre as paletas da mesma conta, o que a chave única composta por \texttt{user\_id} e \texttt{name} garante; usuários diferentes podem ter paletas de mesmo nome. O campo \texttt{subcolors} emprega o tipo JSONB para guardar as sete subcores em ordem fixa, na qual cada posição corresponde a uma região da interface (fundo da página, cartões, títulos, texto, barra lateral, cabeçalho e modais); a quantidade exata é garantida pela validação na camada de aplicação, pois uma paleta é sempre completa (RF-18). O \texttt{system\_theme} guarda o estilo em vigor enquanto a paleta está em uso: nasce do lado para o qual ela foi desenhada e é atualizado quando o usuário altera o estilo no menu da conta. A tipografia (\texttt{font\_family}, \texttt{title\_size}, \texttt{text\_size}, \texttt{line\_height}, \texttt{letter\_spacing}, \texttt{font\_weight} e \texttt{font\_italic}) também é gravada em cada paleta (RF-19). Os dois indicadores booleanos têm papéis distintos: \texttt{is\_active} aponta a paleta em uso, e um índice único parcial garante uma única por usuário; \texttt{is\_default} indica apenas que a paleta foi oferecida pelo sistema no cadastro (RF-01), sem protegê-la de edição ou exclusão. No cadastro, fica ativa a paleta `Claro' ou a `Escuro', conforme o tema do sistema operacional naquele momento, e a conta nunca fica sem paleta porque a paleta em uso não pode ser excluída.
\FloatBarrier
