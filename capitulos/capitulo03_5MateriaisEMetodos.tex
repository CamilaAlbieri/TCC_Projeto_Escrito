...
Diante da quantidade de entidades identificadas, adotou-se a organização do diagrama em cinco pacotes conceituais --- Usuário, Coleções, Editor de Layout, Interação Social, Notificação e Aparência. Esse agrupamento, além de reduzir a sobrecarga visual do diagrama, evidencia a separação de responsabilidades que seria posteriormente materializada na estrutura modular do \textit{backend} implementado com NestJS.

\subsection{Definição arquitetural}

A arquitetura do sistema seguiu o modelo em camadas descrito por \cite{Fowler2002}, com separação clara entre apresentação, lógica de negócio e persistência. A camada de apresentação é implementada em Next.js e comunica-se com o \textit{backend} em NestJS por meio de uma API baseada no estilo REST \cite{Fielding2000}, enquanto este último acessa o PostgreSQL por meio de um ORM que mapeia objetos para tabelas relacionais.
